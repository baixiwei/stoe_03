...
plotSingleGraph = function( type, transparent, idxs=FALSE, first=FALSE, x, y, xlim, ylim, xticks, yticks, xlab, ylab, label=FALSE ) {
    ...
    plotBaseGraph = function( xlim, ylim, xticks, yticks, xlab, ylab, size="large", color="black" ) {
        ...
        if ( size=="small" ) {
            par( las=1, cex=1/3, mar=c(sum(xs),sum(ys),3.5,1.5), oma=c(0,0,0,0) )        
        } else {
...
# decided to keep the ones I'd done in Excel

# xlim = c(0,1); ylim = c(0,.6)
# xticks = seq(0,1,0.2); yticks = seq(0,.6,0.1)
# xlab = "Proportion of successful outcomes"; ylab = "Probability"
...
x = seq(1,10,.1); B = 3; a = 1.0; y = pl(B=B,a=a,x=x); xlab = "Number of trials"; ylab = "Response time (seconds)";
